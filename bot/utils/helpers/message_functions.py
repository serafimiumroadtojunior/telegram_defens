...
async def send_unrestriction_message(
    bot: Bot, chat_id: int, user_id: int, new_datetime: datetime
) -> None:
    ...
    chat_member: Optional[ChatMember] = await bot.get_chat_member(chat_id, user_id)
...
async def optional_keyboard(text: Optional[str] = None, callback_data: Optional[str] = None) -> Optional[InlineKeyboardMarkup]:
...
async def delayed_delete(delay: int, message: Message) -> None:
