...
async def add_reason(tg_id: int, reason: str) -> None:
    async with async_session() as session:
        async with session.begin():
            new_reason = Reasons(tg_id=tg_id, reasons=reason)
...
async def get_user_reasons(tg_id: int) -> Union[List[str], str]:
    async with async_session() as session:
        result: Result = await session.execute(
            select(Reasons.reasons).where(Reasons.tg_id == tg_id)
        )
...
async def delete_user_reason(tg_id: int) -> None:
    async with async_session() as session:
        async with session.begin():
            ...
            await session.execute(delete(Reasons).where(Reasons.id == subquery))
    ...
    async with async_session() as session:
        async with session.begin():
            await session.execute(
                delete(Reasons).where(Reasons.tg_id == tg_id) 
            )
