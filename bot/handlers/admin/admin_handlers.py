...
async def warn_user(message: Message, command: CommandObject):
    ...
    await add_reason(reply.from_user.id, reason)
    warns: int = await check_warns(reply.from_user.id)

    if warns >= 3:
        await reset_warns(reply.from_user.id)
        await delete_user_reasons(reply.from_user.id)
        await ban_with_message(
            bot=bot,
            chat_id=chat.id,
            user_id=reply.from_user.id,
            message_text=f"👀<a href='tg://user?id={reply.from_user.id}'><b>{reply.from_user.first_name}</b></a> has been permanently banned for receiving 3 warnings."
        )
...
async def rewarn_callback_handler(callback_query: CallbackQuery):
    ...
    if user_id_str is not None and callback_query.from_user is not None:
        ...
        await delete_user_reason(user_id)
...
async def get_warns(message: Message):
    if message.chat and message.bot:
        ...
        reasons: Union[List[str], str] = await get_user_reasons(tg_id=reply.from_user.id)
        await answer_message(
            bot=message.bot, 
            chat_id=message.chat.id,
            text=f"🛑Warns for user <a href='tg://user?id={reply.from_user.id}'>{reply.from_user.first_name}</a>: \n{reasons}",
            delay=30
        )
